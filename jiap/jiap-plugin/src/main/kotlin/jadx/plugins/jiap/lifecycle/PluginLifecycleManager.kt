package jadx.plugins.jiap.lifecycle
...
class PluginLifecycleManager(
    ...
    private val onReady: (JiapServer, JiapApi) -> Unit
) {
    private val scheduler = Executors.newSingleThreadScheduledExecutor { r ->
        Thread(r, "JiapPlugin-Scheduler").apply { isDaemon = true }
    }
    ...
    private fun initializeComponents() {
        try {
            ctx.decompiler?.let { decompiler ->
                ...
                val api: JiapApi = JiapApiImpl(decompiler)
                val server = JiapServer(api, port)
                server.start()
                onReady(server, api)
