...
class AndroidAppService(override val decompiler: JadxDecompiler) : JiapServiceInterface {
    ...
    fun handleGetAppManifest(): JiapApiResult {
        try {
            val manifest = getAppManifest() ?: return JiapApiResult(
                success = false,
            ...
            return JiapApiResult(success = false, data = hashMapOf("error" to "handleGetAppManifest: ${e.message}"))
        }
    }

    fun handleGetMainActivity(): JiapApiResult {
        try {
            val manifest = getAppManifest() ?: return JiapApiResult(
                success = false,
            ...
            val mainActivityName = appParams.getMainActivity() ?: return JiapApiResult(
                success = false,
                data = hashMapOf(
            ...
            val mainActivityClass = appParams.getMainActivityJavaClass(decompiler) ?: return JiapApiResult(
                success = false,
                data = hashMapOf(
            ...
            return JiapApiResult(success = false, data = hashMapOf("error" to "handleGetMainActivity: ${e.message}"))
        }
    }

    fun handleGetApplication(): JiapApiResult {
        try {
            val manifest = getAppManifest() ?: return JiapApiResult(
                success = false,
            ...
            val appParams = parser.parse()
            val applicationClass = appParams.getApplicationJavaClass(decompiler) ?: return JiapApiResult(
                success = false,
            ...
            return JiapApiResult(success = false, data = hashMapOf("error" to "handleGetApplication: ${e.message}"))
        }
    }

    fun handleGetExportedComponents(): JiapApiResult {
        try {
            val doc = getManifestDocument() ?: return JiapApiResult(
                false, 
            ...
            return JiapApiResult(true, hashMapOf(
                "type" to "list",
                "count" to components.size,
                "components-list" to components
            ))
        } catch (e: Exception) {
            return JiapApiResult(false, hashMapOf("error" to "handleGetExportedComponents: ${e.message}"))
        }
    }
    ...
    fun handleGetDeepLinks(): JiapApiResult {
        return try {
            ...
            return JiapApiResult(false, hashMapOf("error" to "handleGetDeepLinks: ${e.message}"))
        }
    }

    fun handleGetDynamicReceivers(): JiapApiResult {
        try {
            ...
            return JiapApiResult(false, hashMapOf("error" to "handleGetDynamicReceivers error: ${e.message}"))
        }
    }
    ...
    fun handleGetAllResources(): JiapApiResult {
        return try {
            ...
            return JiapApiResult(false, hashMapOf("error" to "handleGetAllResources: ${e.message}"))
        }
    }
        ...
        return try {
            ...
            for (resFile in resources) {
                if (resFile.deobfName == res) {
                    ...
                    return JiapApiResult(true, hashMapOf("type" to "code", "name" to res, "code" to content))
                }
                if (resFile.deobfName == "resources.arsc") {
            ...
            return JiapApiResult(false, hashMapOf("error" to "handleGetResourceFile: Resource '$res' not found."))
        } catch (e: Exception) {
            return JiapApiResult(false, hashMapOf("error" to "handleGetResourceFile: ${e.message}"))
        }
    }
    ...
    fun handleGetStrings(): JiapApiResult {
        return try {
            ...
            if (stringsList.isEmpty()) return JiapApiResult(false, hashMapOf("error" to "handleGetStrings: No strings.xml resource found."))
            return JiapApiResult(true, hashMapOf("type" to "list", "count" to stringsList.size, "strings-list" to stringsList))
        } catch (e: Exception) {
            return JiapApiResult(false, hashMapOf("error" to "handleGetStrings: ${e.message}"))
        }
    }
}
