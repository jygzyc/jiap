...
class SidecarProcessManager(private var serverPort: Int) {
    ...
    fun start(): Boolean {
        ...
        try {
            ...
            val scriptPath = findScriptPath() ?: run {
                LogUtils.warn("[MCP] Sidecar script not found: decx_mcp_server.py")
                return false
            }
            ...
            val decxUrl = PluginUtils.buildServerUrl(port = serverPort)
            ...
            when (executor) {
                "uv" -> command.addAll(listOf("uv", "run", normalizedScriptPath, "--url", decxUrl))
                else -> command.addAll(listOf(executor, normalizedScriptPath, "--url", decxUrl))
            }
            ...
            pb.environment()["DECX_URL"] = decxUrl
    ...
    private fun findScriptPath(): String? {
        val mcpPath = File(System.getProperty("user.home"), ".decx/mcp")
        ...
        try {
            ...
            val scriptFile = File(mcpPath, "decx_mcp_server.py")
            ...
            extractResource("/mcp/decx_mcp_server.py", scriptFile)
    ...
    fun cleanupMcpFiles() {
        try {
            ...
            if (mcpPath.exists() && mcpPath.isDirectory) {
                LogUtils.info("[MCP] Cleaning up .decx/mcp directory...")
