...
```
1. decx ard app-manifest → 定位 FileProvider 声明
2. decx code implement FileProvider → 查找自定义 FileProvider 实现
3. decx ard all-resources → 列出资源文件，定位 file_paths.xml
4. decx ard resource-file res/xml/file_paths.xml → 获取路径配置内容
5. decx code class-source <FileProviderClass> → 检查 getFileForUri() 路径校验
6. decx code xref-method "package.Class.getUriForFile(...)" → 追踪 URI 权限使用
7. decx code search-class "FLAG_GRANT_READ_URI_PERMISSION" → 搜索 FLAG_GRANT
8. decx code xref-method "android.content.Intent.addFlags(...)" → 追踪 addFlags 调用
