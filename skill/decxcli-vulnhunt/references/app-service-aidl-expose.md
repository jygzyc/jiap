...
2. decx code class-source <ServiceClass> → 检查 onBind() 返回的 Binder/Stub
3. 在 class-source 中查看 IMyService.Stub 匿名类实现（AIDL Stub 实现）
4. decx code implement <AIDL_Interface> → 查找 AIDL 接口的所有实现
5. decx code xref-method "package.Class.onBind(...):IBinder" → 检查权限校验缺失
