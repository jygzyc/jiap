...
```
1. decx code xref-method "com.android.server.am.ActivityManagerService.checkKeyIntent(...)" → 搜索 Bundle 安全校验
2. decx code xref-method "android.content.Intent.filterEquals(...)" → 追踪 filterEquals 调用
3. decx code xref-method "android.content.Intent.filterHasType(...)" → 追踪 filterHasType 调用
4. decx code implement android.os.Parcelable$Creator → 搜索 Bundle 的 Parcelable 实现
5. decx code xref-method "android.os.Parcel.writeBundle(...)" → 追踪 writeBundle 调用
6. decx code class-source <Class> → 获取源码，分析 Bundle 读写路径
7. decx code implement android.os.Parcelable$Creator → 枚举所有 Parcelable 实现
