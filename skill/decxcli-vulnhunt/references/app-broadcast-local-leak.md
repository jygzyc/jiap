...
```
1. decx ard app-manifest → 检查广播接收器声明和权限配置
2. decx code class-source <ReceiverClass> → 获取 Receiver 源码，检查是否调用 sendBroadcast 系列方法
3. decx code xref-method "package.ReceiverClass.sendBroadcast(android.content.Intent):void" → 追踪 sendBroadcast 调用
4. decx code xref-method "package.ReceiverClass.sendOrderedBroadcast(android.content.Intent,java.lang.String):void" → 追踪 sendOrderedBroadcast 调用
5. decx code class-source <Class> → 定位 sendBroadcast 调用，检查 Intent 中的敏感数据
