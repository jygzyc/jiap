...
```
1. decx code class-source <Activity> → 获取源码，搜索 PendingIntent.getActivity/getService/getBroadcast
2. decx code xref-method "android.app.PendingIntent.getActivity(...)" → 交叉引用追踪 PendingIntent 操作
3. decx code xref-method "android.app.PendingIntent.getService(...)" → 追踪 getService 调用
4. decx code xref-method "android.app.PendingIntent.getBroadcast(...)" → 追踪 getBroadcast 调用
5. decx code xref-method "android.app.PendingIntent.send()" → 追踪 send 调用
6. 在反编译源码中搜索 flag 值：
   → 0x2000000 = FLAG_MUTABLE，0x4000000 = FLAG_IMMUTABLE
   → 0x8000000 = FLAG_UPDATE_CURRENT
   → 0xa000000 = FLAG_UPDATE_CURRENT | FLAG_MUTABLE
7. decx code class-source <Activity> → 检查是否使用 FLAG_MUTABLE 且未指定目标组件
