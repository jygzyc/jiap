...
```
1. decx ard exported-components → 定位目标 Activity
2. decx code class-source <Activity> → 搜索 setResult/startActivityForResult/onActivityResult
3. decx code xref-method "package.Class.onActivityResult(...)" → 交叉引用追踪
4. decx code xref-method "package.Class.setResult(...)" → 追踪 setResult 调用
