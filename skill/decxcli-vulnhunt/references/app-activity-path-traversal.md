...
```
1. decx code class-source <Activity> → 获取 Activity 源码，搜索文件操作相关方法
2. 搜索 getData/getDataString/getInputStream/openFileInput/openFileOutput
3. decx code xref-method "package.Class.method(...)" → 定位 getCanonicalPath 等路径操作
4. decx ard exported-components → 定位导出 Activity
5. decx code class-source <Activity> → 定位文件路径参数获取入口
6. 检查路径校验逻辑是否存在
