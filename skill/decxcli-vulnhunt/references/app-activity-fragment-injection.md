...
```
1. decx code implement android.app.Fragment → 检查 Fragment 实现
2. decx code subclass android.app.Fragment → 检查 Fragment 子类
3. decx code class-source <Activity> → 搜索 getSupportFragmentManager/FragmentTransaction/replace/add
4. decx ard exported-components → 定位导出 Activity
5. decx code class-source <Activity> → 检查是否从 Intent 读取 Fragment 类名
6. decx code subclass android.app.Fragment → 枚举应用内部包含敏感操作的 Fragment
