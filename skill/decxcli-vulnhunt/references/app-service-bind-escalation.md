...
```
1. decx ard exported-components → 定位导出服务
2. decx code xref-method "package.Class.bindService(...):boolean" → 搜索 bindService 调用点
3. decx code implement android.content.ServiceConnection → 获取 ServiceConnection 实现类源码
4. decx code class-source <ServiceClass> → 检查 onBind() 返回的 Binder 实现
