...
```
1. decx ard exported-components -P <port>         → 列出所有导出 Activity
2. decx ard main-activity -P <port>               → 定位主入口
3. decx ard app-deeplinks -P <port>               → 检查 Deep Link 入口
4. 对每个导出 Activity：
   a. decx code class-source "<Activity>" -P <port> → 获取源码
   b. 检查 onCreate/onNewIntent 中是否从 Intent 提取嵌套 Intent
   c. 检查是否使用 getParcelableExtra("...*intent*...")
   d. decx code xref-method "<methodSig>" -P <port> → 追踪数据流（签名格式：pkg.Class.method(paramType):returnType）
5. 重点关注：
