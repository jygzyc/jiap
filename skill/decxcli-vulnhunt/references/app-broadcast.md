...
```
1. decx ard receivers -P <port>                    → 列出动态广播接收器
2. decx code search-method "registerReceiver" -P <port> → 定位动态注册点
3. 对每个接收器：
   a. decx code class-source "<Receiver>" -P <port>  → 获取源码
   b. 检查 onReceive 中是否处理敏感数据
   c. 检查是否使用 LocalBroadcastManager（安全）或全局 sendBroadcast（危险）
4. decx code xref-method "android.content.Context.sendBroadcast(android.content.Intent):void" -P <port> → 追踪广播发送
5. decx code xref-method "android.content.Context.sendOrderedBroadcast(android.content.Intent,java.lang.String):void" -P <port> → 有序广播
