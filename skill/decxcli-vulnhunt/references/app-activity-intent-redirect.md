...
```
1. decx ard exported-components → 定位导出 Activity
2. decx code class-source <Activity> → 检查是否从 Intent 读取嵌套 Intent
3. decx code class-source <targetClass> → 搜索 getParcelableExtra 的调用
4. decx code xref-method "package.Class.onCreate(android.os.Bundle):void" → 从生命周期追踪数据流
5. decx code xref-method "com.android.server...checkKeyIntent(...)" → 系统服务 Intent 中转
6. decx code xref-method "package.Class.startActivity(android.content.Intent):void" → 追踪转发调用
