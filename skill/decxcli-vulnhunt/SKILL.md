...
description: Android 漏洞挖掘专用 skill。基于 DECX CLI + JADX，提供组件级攻击面分析、可利用性评估、漏洞链构造方法论和风险评级参考。当用户提到漏洞挖掘、vulnerability hunting、exploit chain、攻击面分析、attack surface、安全审计、security audit、风险评级时使用。
metadata:
  requires:
    bins: ["decx"]
---

# DECX CLI — Android 漏洞挖掘

Android 组件级漏洞挖掘方法论。漏洞发现和评估由本 skill 完成，PoC 构造交由 `decxcli-poc`。

命令参考见 `decxcli`。

## 命令格式强制规则

> 来自 `decxcli` skill，必须遵守：
> - 所有 `decx code` / `decx ard` 命令必须携带 `-P <port>`
> - 方法签名格式：`"package.Class.methodName(paramType1,paramType2):returnType"`，**禁止 `...`**，**必须用引号包裹**（括号/冒号会导致 shell 解析错误）
> - 不确定命令时先 `--help`，不要猜测

## 上下文管理与 Subagent 续接

所有 Subagent 在执行过程中必须监控上下文使用量。当上下文达到 **60%** 时，必须立即执行交接。

### 交接协议

1. Subagent 停止当前分析，输出交接摘要后终止
2. 主 agent 收到后创建新 Subagent，将交接摘要作为输入
3. 新 Subagent 从断点处继续，重复此过程直到任务完成
...
```json
{
  "handoff": true,
  "phase": "<当前 Phase 编号>",
  "component": "<当前分析的组件，Phase 2 为 null>",
  "port": 31234,
  "done": "<已完成的步骤，简述>",
  "next": "<下一步要做什么，新 Subagent 的入口指令>",
  "context": "<新 Subagent 继续所需的上下文数据，如 chain、partialResults 等>"
}
```

- `done`：已完成工作的简要描述，新 Subagent 据此跳过已做步骤
- `next`：新 Subagent 的起始指令，必须足够具体以无缝续接
- `context`：随 Phase 不同而变化，携带续接必需的数据

### 各 Phase context 字段

**Phase 2（侦察）**：已解析的组件列表 + 待执行的命令

**Phase 3（组件分析）**：
```json
{
  "vulnTypesDone": ["Intent 重定向", "Fragment 注入"],
  "vulnTypesPending": ["PendingIntent 滥用", "Task 劫持"],
  "activeChain": {
    "vulnType": "Intent 重定向",
    "chain": [{"method": "...", "finding": "..."}],
    "nextMethod": "com.example.MyActivity.startForwardActivity(android.content.Intent):void"
  },
  "completedFindings": [{"vulnType": "...", "risk": "...", "callChain": [...]}]
}
```

**Phase 4（跨组件）**：跨组件追踪链 + 当前链进度

**Phase 6（报告）**：已完成的章节 + 待生成的章节 + findings 汇总
...
```
decx process open "<apk-path>" -P <port>  → 打开 APK（端口建议 25000–65535）
decx process status -P <port>              → 确认服务正常
```

分析完成后不要自动关闭 session，告知用户：`decx process close "<name>" -P <port>`

### Phase 2：攻击面枚举（侦察 Subagent）

创建一个侦察 Subagent 完成全部信息收集，主 agent **不执行任何 `decx` 命令**，仅接收结构化输出。
...
```
decx ard exported-components -P <port>  → 导出组件列表
decx ard app-deeplinks -P <port>        → Deep Link 入口
decx ard app-receivers -P <port>        → 动态广播接收器
decx ard app-manifest -P <port>         → 完整 Manifest
decx ard get-aidl -P <port>            → AIDL 接口
decx ard strings -P <port>              → 硬编码字符串
```
...
- 侦察 subagent 执行全部 `decx` 命令并完成分析，主 agent **不直接调用任何 Phase 2 命令**
- 所有命令携带 `-P <port>`
...
```
1. decx code method-source "<currentMethod>" -P <port>  → 获取该方法源码
2. 分析源码，执行以下判断：
```
...
3. 使用 `decx code method-source "<method>" -P <port>` 回取关键代码片段（仅 Source / Sink / 校验缺失位置）
4. **严格按模板结构逐项填充，生成报告**
...
1. 传递给 `decxcli-poc` 构造 PoC 验证（session 需保持运行）
2. 告知用户可通过 `decx process close "<name>" -P <port>` 手动关闭 session，或直接使用 `decxcli-poc` skill
