...
- ContentProvider 覆写了 `call()` 方法
- `call()` 方法暴露了敏感操作（如文件操作、命令执行）
- `call()` 方法内部的权限校验比 CRUD 方法更弱或完全缺失（系统不会自动检查 Manifest 中定义的权限）
- `android:exported="true"` 且未设置 `android:permission`
- `call()` 方法可用于获取系统服务中的远程对象（如 PendingIntent），进一步构造伪造广播或启动任意组件
- `call()` 方法接受外部传入的路径参数时，可能加载外部 DEX 文件导致特权进程代码执行
...
1. jiap ard exported-components → 定位导出 Provider
2. jiap code class-source <ProviderClass> → 检查是否覆写 call() 方法
3. 分析 call() 方法的 switch/if 分支，识别敏感操作
4. 检查是否缺少权限校验（对比 query/insert 的校验逻辑）
5. adb shell content call --uri content://<authority> --method <method> --extra <params>
6. 触发敏感操作（文件操作、命令执行、数据返回）
```
