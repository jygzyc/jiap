...
```
1. jiap code xref-method "android.os.Parcel.readSerializable()" → 搜索不安全的反序列化调用
2. jiap code xref-method "android.os.Bundle.readSerializable()" → 追踪 Bundle.readSerializable 调用
3. jiap code xref-method "android.os.Parcel.readValue()" → 追踪 readValue 调用
4. jiap code xref-method "android.content.Intent.getSerializableExtra(...)" → 搜索 getSerializableExtra 调用
5. jiap code class-source <Class> → 获取源码，追踪数据流
6. jiap code xref-method "java.lang.ClassLoader.loadClass(...)" → 检查是否存在白名单校验
7. jiap code xref-method "android.content.Intent.getSerializableExtra(...)" → 定位反序列化入口
