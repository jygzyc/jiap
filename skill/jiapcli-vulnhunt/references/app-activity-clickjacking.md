...
```
1. jiap ard app-manifest → 检查 manifest 和权限
2. jiap code xref-field "android.Manifest.permission.SYSTEM_ALERT_WINDOW" → 检查触摸安全相关
3. jiap code xref-field "android.view.WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY" → 检查覆盖层使用
4. jiap code subclass android.app.Activity → 检查是否覆写了 onFilterTouchEventForSecurity
