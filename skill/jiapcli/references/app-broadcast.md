...
```
1. jiap ard receivers                    → 列出动态广播接收器
2. jiap code search-method "registerReceiver" → 定位动态注册点
3. 对每个接收器：
   a. jiap code class-source <Receiver>  → 获取源码
   b. 检查 onReceive 中是否处理敏感数据
   c. 检查是否使用 LocalBroadcastManager（安全）或全局 sendBroadcast（危险）
4. jiap code xref-method "android.content.Context.sendBroadcast(...)" → 追踪广播发送
5. jiap code xref-method "android.content.Context.sendOrderedBroadcast(...)" → 有序广播
