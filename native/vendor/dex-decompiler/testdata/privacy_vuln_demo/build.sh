#!/usr/bin/env bash
# Rebuild privacy_vuln_demo.apk + classes.dex from the Java sources in this folder.
...
BUILD_TOOLS=""
# Prefer the highest versioned build-tools dir that contains d8 + aapt2 + apksigner.
...
echo "==> javac"
# --release 8 keeps bytecode simple (no invokedynamic for try-with-resources, etc.).
find "$ROOT/java" "$ROOT/stubs" -name '*.java' -print0 \
...
if [ ! -f "$KS" ]; then
  ...
  keytool -genkeypair -keystore "$KS" -storepass android -alias androiddebugkey \
    ...
    -dname "CN=Android Debug,O=AndroHunt,C=US"
fi
...
cp "$WORK/signed.apk" "$ROOT/privacy_vuln_demo.apk"

echo "wrote $ROOT/classes.dex ($(wc -c < "$ROOT/classes.dex") bytes)"
echo "wrote $ROOT/privacy_vuln_demo.apk ($(wc -c < "$ROOT/privacy_vuln_demo.apk") bytes)"
