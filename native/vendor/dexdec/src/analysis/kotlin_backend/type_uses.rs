...
impl ClassTypeUses {
    pub(super) fn collect(class: &KotlinClassModel) -> Vec<ArgType> {
    ...
    fn collect_class(class: &KotlinClassModel, types: &mut Vec<ArgType>) {
        ...
        for method in &class.methods {
            KotlinMethodTypeUses::collect_into(method, types);
        }
    }
...
pub(super) struct KotlinMethodTypeUses;

impl KotlinMethodTypeUses {
    pub(super) fn collect(method: &super::kotlin_model::KotlinMethodModel) -> Vec<ArgType> {
        let mut types = Vec::new();
        Self::collect_into(method, &mut types);
        types
    }

    fn collect_into(method: &super::kotlin_model::KotlinMethodModel, types: &mut Vec<ArgType>) {
...
impl AnnotationTypeUses {
    pub(super) fn collect(annotations: &[AnnotationNode], types: &mut Vec<ArgType>) {
        for annotation in annotations {
            if crate::frontend::kotlin_metadata::KotlinMetadata::is_metadata(annotation) {
                continue;
            }
