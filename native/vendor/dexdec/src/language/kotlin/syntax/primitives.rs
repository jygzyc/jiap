//! Primitive operation domains recovered from erased DEX values.

use crate::ir::{ArithOp, PrimitiveType};

/// Computes the JVM operation domain that DEX instructions encode after
/// source-level narrow primitive types have been erased.
pub(in crate::language::kotlin) struct PrimitiveOperationDomain;

impl PrimitiveOperationDomain {
    pub(in crate::language::kotlin) fn numeric(primitive: PrimitiveType) -> Option<PrimitiveType> {
        matches!(
            primitive,
            PrimitiveType::Byte
                | PrimitiveType::Short
    ...
    pub(in crate::language::kotlin) fn unary_numeric_promotion(
    ...
    pub(in crate::language::kotlin) fn binary_numeric_promotion(
        left: PrimitiveType,
        right: PrimitiveType,
    ) -> Option<PrimitiveType> {
    ...
    pub(in crate::language::kotlin) fn arithmetic_result(
        operator: ArithOp,
        left: PrimitiveType,
        right: PrimitiveType,
    ) -> Option<PrimitiveType> {
    ...
    pub(in crate::language::kotlin) fn is_widening(
