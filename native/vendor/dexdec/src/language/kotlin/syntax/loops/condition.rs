//! Inlining of loop-test setup values into Kotlin condition expressions.
...
impl LoopRotation {
    ...
    /// Rotates a side-effecting loop header into Kotlin's pre-tested form:
    ///
    /// `loop { setup; if (!condition) break; body; }`
    /// becomes `setup; while (condition) { body; setup; }`.
    ///
    /// A local continue would bypass the duplicated setup in Kotlin, so such a
...
/// Kotlin's internal counted-loop form is valid for a pre-tested semantic loop
/// whose test has one setup evaluation. A local `continue` cannot be lowered to
/// a lexical tail update without changing its cleanup order, so those loops
/// retain their explicit test setup.
...
impl LoopTestCycle {
    pub(super) fn apply(node: SemanticNode) -> SemanticNode {
        ...
        if Self::has_local_continue(control, &body) {
            return SemanticNode::Loop {
                control,
                header,
                kind: SemanticLoopKind::PreTested,
    ...
    fn has_local_continue(control: SemanticLoopControl, body: &SemanticNode) -> bool {
        let completion = crate::ir::semantic::SemanticCompletion::analyze(body);
        match control {
            SemanticLoopControl::Region(region) => completion.has_continue_to_region(region),
            SemanticLoopControl::Label(label) => completion.has_continue_to_label(label),
        }
