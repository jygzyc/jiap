...
use crate::language::java::ast::JavaIdentifier;

use super::JavaStructuralError;
...
impl ControlLayout {
    pub(super) fn analyze(root: &SemanticNode) -> Result<Self, JavaStructuralError> {
        LabelAnalysis::analyze(root)
    }

    pub(super) fn label(&self, region: Option<RegionId>) -> Option<JavaIdentifier> {
        region
            .filter(|region| self.regions.contains(region))
            .map(Self::region_identifier)
    }

    pub(super) fn loop_label(&self, control: SemanticLoopControl) -> Option<JavaIdentifier> {
        match control {
    ...
    pub(super) fn leave_label(&self, region: RegionId) -> Option<JavaIdentifier> {
        self.regions
            .contains(&region)
            .then(|| Self::region_identifier(region))
    }

    pub(super) fn label_for(&self, label: SemanticLabel) -> Option<JavaIdentifier> {
        self.labels
            .contains(&label)
            .then(|| Self::label_identifier(label))
    }

    pub(super) fn label_identifier(label: SemanticLabel) -> JavaIdentifier {
        let category = match label.kind {
            SemanticLabelKind::Block => "flow",
            SemanticLabelKind::Loop => "loop",
        };
        JavaIdentifier::from_dex(&format!(
            "{}_{}_{}",
            category,
            label.region.raw(),
            label.block.raw()
    ...
    fn region_identifier(region: RegionId) -> JavaIdentifier {
        JavaIdentifier::from_dex(&format!("region_{}", region.raw()))
    }
}
...
struct LabelAnalysis {
    ...
    regions: BTreeSet<RegionId>,
    labels: BTreeSet<SemanticLabel>,
    invalid: Option<JavaStructuralError>,
}
...
impl LabelAnalysis {
    fn analyze(root: &SemanticNode) -> Result<ControlLayout, JavaStructuralError> {
        ...
        match analysis.invalid {
            ...
            None => Err(JavaStructuralError::MissingControlScope),
        }
    }
    ...
    fn record_region(&mut self, target: RegionId, direction: ControlDirection) {
        ...
        let Some(index) = self
            ...
            self.invalid
                .get_or_insert(JavaStructuralError::UnknownControlTarget(target));
            return;
        };
    ...
    fn record_label(&mut self, target: SemanticLabel, direction: ControlDirection) {
        ...
        let Some(index) = index else {
            self.invalid
                .get_or_insert(JavaStructuralError::MissingControlScope);
            return;
        };
    ...
    fn record_leave(&mut self, node: &SemanticNode) {
        ...
        match &leave.kind {
            ...
            SemanticLeaveKind::FallThrough(target) | SemanticLeaveKind::Jump(target) => {
                self.invalid
                    .get_or_insert(JavaStructuralError::UnboundContinuation {
                        scope: leave.target,
                        target: *target,
                    });
            }
