...
impl DexSymbolsCodec {
    ...
    pub fn encode(database: &PlatformSymbolDatabase) -> io::Result<Vec<u8>> {
        ...
        // Native-port patch: compressed storage needs the C zstd backend
        // (feature `symbol-encode`); without it, store raw — the reader
        // accepts both forms.
        #[cfg(feature = "symbol-encode")]
        let (flags, stored) = {
            let stored = zstd::stream::encode_all(Cursor::new(&raw), 8)?;
            (Self::FLAG_ZSTD, stored)
        };
        #[cfg(not(feature = "symbol-encode"))]
        let (flags, stored) = (0, raw.clone());
    ...
    pub fn decode(bytes: &[u8]) -> io::Result<PlatformSymbolDatabase> {
        ...
        let raw = if flags & Self::FLAG_ZSTD != 0 {
            // Native-port patch: decode with pure-Rust ruzstd instead of the
            // C zstd backend.
            let mut decoder = ruzstd::decoding::StreamingDecoder::new(Cursor::new(stored))
                .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, format!("dexsym zstd: {e}")))?;
