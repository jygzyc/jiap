...
pub struct DexArchive {
    archive: decx_apk::ZipArchive,
    entries: std::vec::IntoIter<String>,
}

impl DexArchive {
    pub fn open(filepath: &str) -> Result<Self, DexError> {
        // decx-native: reads were replaced by `decx_apk` (pure std zip reader)
        let raw = std::fs::read(filepath)
            .map_err(|e| DexError::InvalidArchive(format!("{}: {e}", filepath)))?;
        let archive =
            decx_apk::ZipArchive::open(raw).map_err(DexError::InvalidArchive)?;
        let entries = archive
            .entry_names()
            .into_iter()
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        Some((|| {
            let raw_dex = self
                ...
                .read_by_name(&entry)
                .ok_or_else(|| {
                    DexError::InvalidArchive(format!("entry not found: {}", entry))
                })?
                .map_err(DexError::InvalidArchive)?;
...
impl DexReader {
    ...
    pub fn read_uleb128(&mut self) -> Result<(u32, usize), DexError> {
        ...
        self.bytes.read_exact(&mut byte_buf)?;
        let byte = byte_buf[0];
            bytes_read += 1;
