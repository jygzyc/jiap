...
use crate::rusty_dex::dex::access_flags::{AccessFlag, AccessFlagType};
use crate::rusty_dex::dex::code_item::CodeItem;
use crate::rusty_dex::dex::encoded_value::{
...
use crate::rusty_dex::dex::fields::DexFields;
use crate::rusty_dex::dex::methods::DexMethods;
...
/// Extract `name` from a prototype `Lclass;->name(args)ret`.
/// (decx-native: replaces the upstream `regex`/`lazy_static` dependency.)
fn method_name_from_proto(proto: &str) -> &str {
    match proto.find("->") {
        Some(i) => {
            let rest = &proto[i + 2..];
            match rest.find('(') {
...
impl EncodedMethod {
    ...
    pub fn get_method_name(&self) -> &str {
        let method_name = method_name_from_proto(&self.proto);
